...
export const DuolingoFireWidget: React.FC<DuolingoFireWidgetProps> = ({
  ...
  const {
    ...
    fallbackData: {
      ...
      totalXp: fallbackStats.totalXp,
      level: fallbackStats.level,
      learningLanguage: fallbackStats.learningLanguage,
      avatar: fallbackStats.avatar,
    },
  });
  ...
  const displayStats = hookStats
    ? {
        ...hookStats,
        courses: fallbackStats.courses, // Keep static courses for now as hook doesn't fetch them
        motivation: fallbackStats.motivation,
      }
    : null;
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-col flex-grow">
          ...
          <div className="flex justify-between items-end">
            <div className="flex flex-col">
              <span
                className={`text-[0.8rem] font-bold uppercase tracking-wider ${isFireActive ? 'text-white/90' : 'text-gray-500'}`}
              >
                Streak
              </span>
      ...
      <div
        className={`relative z-10 border-t ${isFireActive ? 'border-white/20' : 'border-gray-200'} pt-3 mt-1`}
      >
        <div className="flex justify-between items-center mb-2">
          <span
            className={`text-[0.8rem] font-bold uppercase ${isFireActive ? 'text-white/90' : 'text-gray-500'}`}
          >
            Active Courses
          </span>
        </div>
        ...
        <div className="space-y-2">
          {displayStats?.courses.slice(0, 3).map((course, idx) => (
            <div key={idx} className="flex justify-between items-center text-[1.15rem]">
              ...
              <span
                className={`font-mono text-[0.9rem] ${isFireActive ? 'text-white/80' : 'text-gray-500'}`}
              >
