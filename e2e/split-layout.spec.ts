...
test.describe('Split Layout', () => {
  ...
  test('should toggle between creative and formal views', async ({ page }) => {
    await page.goto('/');
    
    // Default should be creative
    await expect(page.locator('text=Creative Space')).toBeVisible();
    ...
    await expect(page.locator('text=Creative Space')).not.toBeVisible();
  ...
  test('should persist side preference in URL', async ({ page }) => {
    ...
    const toggleButton = page.getByLabel(/switch to/i);
    await toggleButton.click();
    await expect(page).toHaveURL(/side=creative/);
  });
  ...
  test('should be keyboard accessible', async ({ page }) => {
    ...
    await expect(focusedElement).toHaveText(/Formal|Creative/);
